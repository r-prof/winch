...
`rlang::trace_back()` is a wrapper that displays as a tree and creates better output in the presence of lazy evaluation.
In our simple example, this doesn't matter, the tree is a sequence.
...
(It is much more surprising that it allows programs to compute on that execution state -- on the other hand, R has very strong reflection abilities.)
...
The winch package wraps two libraries (for portability across different OSs) that help obtain IP sequences and function names.
This functionality is accessible via `winch::winch_trace_back()`.
For demonstration purposes, this is what the native stack trace looks like when this document is rendered.
The function names are mostly internal to R, this means we're running only interpreted (or JIT-compiled) R code:
